  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 10000 }} role="dialog" aria-modal="true">
      <div style={{ background: 'radial-gradient(circle at center, #1a1a1a, #000)', padding: '32px', borderRadius: '16px', textAlign: 'center', color: '#fff', maxWidth: '400px', width: '90%', boxShadow: '0 0 30px rgba(255,255,255,0.1)' }}>
        <h2 style={{ marginBottom: '16px', fontSize: '22px' }}>How to use the Premium Blocks</h2>
        <p style={{ marginBottom: '12px', fontSize: '16px', color: '#ddd' }}>
          Go to your <strong>Theme Editor</strong> and click on <strong>Add Block</strong> in the App section.
        </p>
        <p style={{ marginBottom: '24px', fontSize: '14px', color: '#ccc' }}>
          Choose any premium block: Announcement Bar, Popup, or Countdown, and customize it freely!
        </p>
...
function PreviewAnnouncementBar() {
  const bars = [
    ...
    { bg: 'linear-gradient(to right, #0f38ef, #89ffe1)', color: '#fff', text: 'Flash Sale Alert! Everything Must Go – Save Big Before It’s Gone!', buttonText: 'Grab Deal', link: '#' },
    { bg: 'linear-gradient(to right, #13eb28, #a3e8ec)', color: '#000', text: 'Clearance – Prices Slashed! Don\'t Miss Out on Major Savings!', buttonText: 'Browse', link: '#' },
  ];
  return (
...
// PreviewCountdown with correct signature
...
export default function Settings() {
  ...
  return (
    <>
          ...
          <div key={block.id} style={CARD_STYLE}>
            <div style={{ flex: 1, minWidth: '220px' }}>
              ...
              <a href={`${baseEditorUrl}&addAppBlockId=be79dab79ff6bb4be47d4e66577b6c50/${block.id}`} target="_blank" rel="noopener noreferrer">
                <button style={{ ...BUTTON_BASE, backgroundColor: '#000', color: '#fff' }}>Add Premium Block</button>
              </a>
            </div>
      ...
      <Link to="/pricing">
        <button style={{ position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%)', ...BUTTON_BASE, backgroundColor: '#000', color: '#fff', padding: '12px 28px', borderRadius: '30px' }}>
          Pricing
        </button>
      </Link>
      <a href="https://wa.me/212612345678" target="_blank" rel="noopener noreferrer" style={{ position: 'fixed', bottom: '24px', left: '24px', backgroundColor: '#000', borderRadius: '50%', padding: '14px', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}>
